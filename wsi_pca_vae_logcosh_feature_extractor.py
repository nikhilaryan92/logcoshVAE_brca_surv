...
tcga_id = df_wsi[df_wsi.columns[0]]
class_labels = df_wsi[df_wsi.columns[len(df_wsi.columns)-1]]
# =============================================================================
df_wsi = df_wsi.drop(df_wsi.columns[0], axis=1) # droping tcga id from first column
X = df_wsi.drop(df_wsi.columns[len(df_wsi.columns)-1], axis=1).values # dropping class label from last column
...
df_wsi_pca = pd.DataFrame(X_transformed, columns = columns)
plt.plot(np.cumsum(pca.explained_variance_ratio_))
plt.xlabel('number of components')
plt.ylabel('Explained variance')
plt.savefig('elbow_plot.png',dpi=100)
df_wsi_pca.insert(0, "submitter_id.samples", tcga_id.values, True)
df_wsi_pca.insert(loc = len(df_wsi_pca.columns),column = 'label_wsi',value = class_labels.values)
output_file_name = 'pca_features_wsi.csv'
df_wsi_pca.to_csv(os.path.join(path,'wsi',output_file_name),index=False)
...
latent_df.insert(loc = len(latent_df.columns),column = 'label_wsi',value = class_labels.values)
latent_df.to_csv(os.path.join(path,'wsi','vae_features_wsi.csv'),index=False,header=True)
