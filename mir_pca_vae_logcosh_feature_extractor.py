...
tcga_id = df_mir[df_mir.columns[0]]
class_labels = df_mir[df_mir.columns[len(df_mir.columns)-1]]
# =============================================================================
df_mir = df_mir.drop(df_mir.columns[0], axis=1) # droping tcga id from first column
X = df_mir.drop(df_mir.columns[len(df_mir.columns)-1], axis=1).values # dropping class label from last column
...
df_miR_pca = pd.DataFrame(X_transformed, columns = columns)
plt.plot(np.cumsum(pca.explained_variance_ratio_))
plt.xlabel('number of components')
plt.ylabel('Explained variance')
plt.savefig('elbow_plot.png',dpi=100)

df_miR_pca.insert(0, "submitter_id.samples", tcga_id.values, True)
df_miR_pca.insert(loc = len(df_miR_pca.columns),column = 'label_mir',value = class_labels.values)
output_file_name = 'pca_features_mir.csv'
df_miR_pca.to_csv(os.path.join(path,'miRSeq',output_file_name),index=False)
...
latent_df.insert(loc = len(latent_df.columns),column = 'label_mir',value = class_labels.values)
latent_df.to_csv(os.path.join(path,'miRSeq','vae_features_mir.csv'),index=False,header=True)
