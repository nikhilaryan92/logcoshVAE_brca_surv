...
tcga_id = df_dna[df_dna.columns[0]]
class_labels = df_dna[df_dna.columns[len(df_dna.columns)-1]]
# =============================================================================
df_dna = df_dna.drop(df_dna.columns[0], axis=1) # droping tcga id from first column
X = df_dna.drop(df_dna.columns[len(df_dna.columns)-1], axis=1).values # dropping class label from last column
...
df_dna_pca = pd.DataFrame(X_transformed, columns = columns)
plt.plot(np.cumsum(pca.explained_variance_ratio_))
plt.xlabel('number of components')
plt.ylabel('Explained variance')
plt.savefig('elbow_plot.png',dpi=100)
df_dna_pca.insert(0, "submitter_id.samples", tcga_id.values, True)
df_dna_pca.insert(loc = len(df_dna_pca.columns),column = 'label_dna',value = class_labels.values)
output_file_name = 'pca_features_dna.csv'
df_dna_pca.to_csv(os.path.join(path,'dna_methylation',output_file_name),index=False)
...
latent_df.insert(loc = len(latent_df.columns),column = 'label_dna',value = class_labels.values)
latent_df.to_csv(os.path.join(path,'dna_methylation','vae_features_dna.csv'),index=False,header=True)
