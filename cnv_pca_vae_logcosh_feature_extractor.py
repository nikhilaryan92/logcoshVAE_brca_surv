...
tcga_id = df_cnv[df_cnv.columns[0]]
class_labels = df_cnv[df_cnv.columns[len(df_cnv.columns)-1]]
# =============================================================================
df_cnv = df_cnv.drop(df_cnv.columns[0], axis=1) # droping tcga id from first column
X = df_cnv.drop(df_cnv.columns[len(df_cnv.columns)-1], axis=1).values # dropping class label from last column
...
df_cnv_pca = pd.DataFrame(X_transformed, columns = columns)
plt.plot(np.cumsum(pca.explained_variance_ratio_))
plt.xlabel('number of components')
plt.ylabel('Explained variance')
plt.savefig('elbow_plot.png',dpi=100)
df_cnv_pca.insert(0, "submitter_id.samples", tcga_id.values, True)
df_cnv_pca.insert(loc = len(df_cnv_pca.columns),column = 'label_cnv',value = class_labels.values)
output_file_name = 'pca_features_cnv.csv'
df_cnv_pca.to_csv(os.path.join(path,'cnv',output_file_name),index=False)
...
latent_df.insert(loc = len(latent_df.columns),column = 'label_cnv',value = class_labels.values)
latent_df.to_csv(os.path.join(path,'cnv','vae_features_cnv.csv'),index=False,header=True)
